...
-   see how means differ, size of distribution, and how many observations, skew, etc

-   used for statistics and qualitative visual observations

-   axis to manipulate: x-axis

    -   the bins ( less bins less details ; more bins - can't see trends if bins are too broad or fine)

    -   how to decide bin width?

### Density plots

-   proportional amount of data representative of the whole data set

-   x-axis is not as manipulatable

-   should all equal 1

### Ridgeline plots

advantages: easily see distribution for each groups you're plotting for, comparing with more visibility; plotted on different planes of the same axis

can plot values over time

weakness: don't know what the peaks represent

in caption describe the difference ; make a supplemental figure with each species/group separately

### Box plot

shows: summary statistics (line is median is 50 percentile)

### Violin

wider violin- more points; lost the quantile and median

how can we accomplish this? violin overlayed on the box plot ; only need one y-axis because it is one set of values
...
drought %>% 
  ggplot(aes(x = state_abb, y=area_pct))+
  geom_violin() + 
  coord_flip()
```
